...
if ($current_price < $bid_price){
    ...
    $watchlist_users = "SELECT users.email, users.first_name, users.user_id FROM users INNER JOIN watch ON watch.user_id=users.user_id WHERE watch.auction_id = {$item_id}";
    ...
    foreach ($usersObj as $row) {
            ...
            if ($userID != $buyer_id){
                all_watch_lists($firstName, $email, $item_id);
            }
        }
        
    // add to the watchlist (if he isn't watching it already)
    $watching = FALSE;
    $check_watching = "SELECT * FROM `watch` WHERE `user_id` = {$buyer_id} AND `auction_id` = {$item_id}";
    $watchObj = $connection->query($check_watching)->fetch_assoc();
        
    if (!$watchObj){
        $query = "INSERT INTO `watch` (`watch_id`, `user_id`, `auction_id`) VALUES (NULL, {$buyer_id}, {$item_id})";
        $resultObj = $connection->query($query);
    }
